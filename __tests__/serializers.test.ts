import { deserializeBed, deserializeSettings, deserializeBedEvent } from '../src/services/storage/serializers';
...
describe('deserializeBedEvent', () => {
  it('returns null for incomplete event', () => {
    expect(deserializeBedEvent({ id: 'x' })).toBeNull();
  });

  it('returns null for null input', () => {
    expect(deserializeBedEvent(null)).toBeNull();
  });

  it('ensures penalty is non-negative', () => {
    const raw = { id: 'e1', type: 'pet', label: 'Pet', penalty: -5, createdAt: '2024-01-01T00:00:00.000Z' };
    const result = deserializeBedEvent(raw);
    expect(result?.penalty).toBe(0);
  });

  it('deserializes valid event correctly', () => {
...
describe('deserializeQueue', () => {
  ...
  it('returns valid mutations and filters invalid ones', () => {
    ...
    const input = [
      ...
      { id: 'm2' }, // missing required fields — filtered out
      null,
    ];
